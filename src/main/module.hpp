...
namespace cppm {
	...
	class Module {
		...
		protected:
			...
			bool sendMessage(SocketType sockT, Message wMsg)
			{
				bool sendOk = false;
			...
			bool sendMessageRecv(SocketType sockT, Message wMsg, function<bool(const  Message&)> callback)
			{
				bool sendOk = false;
				if (sockT == SocketType::PUB || sockT == SocketType::SUB ||
			...
			retType recvMessage(SocketType sockT,
								function<retType(const Message&)> callback,
								long timeout=1000
			) {
				Message wMsg;
